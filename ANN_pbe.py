...
pbe_input_chunk_location = 'C:/Users/G_Subramanian/OneDrive - Dell Technologies/Documents/probablistic matching/test_files/merged_input_data.ftr'
pbe_reference_chunk_location ='C:/Users/G_Subramanian/OneDrive - Dell Technologies/Documents/probablistic matching/test_files/merged_reference_data.ftr'
...
reference_sample_data = {
    ...
    'ip': [
        '197.250.100.14','209.17.40.36|202.212.64.201', '202.212.64.201', '136.226.48.118', '128.119.202.127', 
        '73.213.248.187'
    ],
    'ua_tokenized': [
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.0', 
...
def create_minhash_signature(data: str, num_perm: int=32):
    minhash = MinHash(num_perm=num_perm)
    for token in data.split():
        minhash.update(token.encode('utf8'))
    return minhash # need to digest the result
    ...
    for attribute_name, attribute_config in evaluation_configuration.items():
        ...
        try:
            ...
            if matching_mode in ["PRECISE", "FUZZY"]:
                ...
                #print (df_matched[f'pbe_{attribute_name}_union'])
            ...
            #print(f'pbe_{attribute_name}_score_weighted')
...
def process_data(pbe_input, pbe_reference):
    ...
    print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ':: Generating prematch keys')   
    # evaluation_attributes = ['ip']
    # prematch_key_columns = sorted(list(map(lambda x: x, evaluation_attributes)))
    ...
    print(pbe_reference['pbe_prematch_key'])
    print(pbe_input['pbe_prematch_key'])
    #print (pbe_reference['pbe_ip'])
    print (pbe_reference['pbe_ua_tokenized'])
    print (pbe_input['pbe_ua_tokenized'])
    #print (pbe_reference['pbe_device_info'])
    
    
    # Create LSH Forest for approximate nearest neighbor search 
    num_perm = 32
    lsh_forest = MinHashLSHForest(num_perm=num_perm, l = 8)
    
    print('Creating MinHash signatures for REFERENCE data...')
    reference_signatures = {}
    for idx, row in pbe_reference.iterrows():
        minhash = create_minhash_signature(row['pbe_prematch_key'])
        #print(minhash)
        lsh_forest.add(str(idx), minhash) 
        reference_signatures[str(idx)] = minhash  
    ...
    for batch in range(batches_total):
        ...
        # # Define batch range
        # batch_index_min = batch * batch_size
        ...
        # Extract the batch of input data
        ...
        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ':: INPUT data processing started')

        # Create MinHash signatures for the input data
        minhashes = []
        for idx, row in tmp_pbe_input.iterrows():
            minhash = create_minhash_signature(row['pbe_prematch_key'])
            minhashes.append((idx, minhash))
        
        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ':: INPUT MinHash signatures created')

        # ANN search using LSH Forest and calculate Jaccard similarity
        for idx, minhash in minhashes:
            results = []
            result_indices = lsh_forest.query(minhash, 5)
            #print (result_indices)
            #print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + '::  ANN step completed')
            for ref_idx in result_indices:
                ref_idx_str = str(ref_idx)  # Ensure index is a string
                if ref_idx_str in reference_signatures:
                    similarity = minhash.jaccard(reference_signatures[ref_idx_str])
                    results.append({'input_index': idx, 'reference_index': ref_idx_str, 'similarity': similarity})
                else:
                    print(f"Reference index {ref_idx_str} not found in reference_signatures")
            
            # Sort results by similarity in descending order
            results = sorted(results, key=lambda x: x['similarity'], reverse=True)
            results_list.extend(results)

        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ':: ANN search and similarity calculation completed')

        tmp_pbe_prematch_dataset = pd.DataFrame(results_list)
        #tmp_pbe_prematch_dataset.to_csv('C:/Users/G_Subramanian/OneDrive - Dell Technologies/Documents/probablistic matching/test_files/ANN_result.csv')
        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ':: Results DataFrame created')
    ...
    #return tmp_pbe_prematch_dataset

if __name__ == "__main__":
    pbe_match_dataset = process_data(pbe_input, pbe_reference)
    print (pbe_match_dataset)
    #pbe_match_dataset.to_csv('C:/Users/G_Subramanian/OneDrive - Dell Technologies/Documents/probablistic matching/test_files/ANN_PBE_result_mergeddata_02_09_ip_ua_device.csv')
    ...
    # # Print the corresponding IPs and similarity scores
    # print(merged_df[['ip_input', 'ip_reference', 'mcvisid_reference', 'record_id_reference', 'all_dellid_reference', 'similarity']])
